window.SmartRosaryLocales = window.SmartRosaryLocales || {};

window.SmartRosaryLocales.de = (FW_VERSION, LANG_VERSION) => ({
  langLabel: "Sprache",
  title: "SmartRosary-Installer",
  usbTitle: `Firmware ${FW_VERSION} per USB hochladen`,
  otaTitle: `Firmware ${FW_VERSION} per Bluetooth hochladen`,
  langTitle: `Sprache${LANG_VERSION} per Bluetooth hochladen`,
  intentsTitle: "Intentionen per Bluetooth hochladen",
  audioTitle: "Audio per Bluetooth hochladen",
  connectUpload: "Verbinden & Hochladen",
  firmwareHardwareLabel: "Hardware:",
  fwLangLabel: "Sprache:",
  intentionsCatalogLoading: "Intentionen werden geladen…",
  intentionsCatalogError: "Intentionen konnten nicht geladen werden. Versuche es erneut, um Pakete oder einzelne Intentionen auszuwählen.",
  intentionsChoose: "Wähle ein oder mehrere Pakete oder einzelne Intentionen.",
  intentionsOptional: "Optional: Wähle Pakete oder einzelne Intentionen zum Übertragen.",
  intentionsSelected: "{count} / 32 Intentionen ausgewählt. Sie werden gemeinsam übertragen.",
  intentionsTooMany: "Wähle insgesamt höchstens 32 Intentionen.",
  intentionsTooLarge: "Der ausgewählte Text überschreitet die Gerätekapazität. Wähle weniger Intentionen.",
  intentionsRetry: "Erneut versuchen",
  intentionsLabel: "Intentionen:",
  audioBackendLabel: "Audioset:",
  audioLanguageLabel: "Audiosprache:",
  audioUsbHint: "Audio wird nur auf dem ESP32-S3 AMOLED-Rosenkranz installiert. Andere Boards ignorieren diese Auswahl.",
  audioBleHint: "Audio-Upload ist nur fur den ESP32-S3 AMOLED-Rosenkranz verfugbar.",
  audioPreviewHint: "Audiostimmen können vor der Installation angehört werden.",
  audioPreviewLink: "Audiovorschau öffnen",
  audioManifestLoading: "Audiopakete werden geladen…",
  audioVersionHint: "Audiopaket v{version}: {label}",
  noneOption: "Keine",
  statusReady: "Bereit.",
  statusConnecting: "Verbinden…",
  statusPreparing: "Verbunden. Übertragung wird vorbereitet…",
  errorNotRosary: "Ausgewähltes Gerät ist kein Rosenkranz (Namensprüfung fehlgeschlagen).",
  errorConsent: "Gerät verweigert Zustimmung oder Zeitüberschreitung. Bitte \"Allow\" am Gerät wählen und erneut versuchen.",
  errorFirmwareMissing: "Firmware-Paket nicht gefunden.",
  errorHardwareUnknown: "Das Gerät hat sein Hardwaremodell nicht gemeldet. Das Bluetooth-Firmwareupdate wurde gestoppt, damit keine falsche Firmware installiert wird. Aktualisieren Sie einmal per USB mit der richtigen Hardwareauswahl.",
  unknownHardwareLabel: "unbekannte Hardware",
  errorIntentionsMissing: "Intentionen-Datei nicht gefunden.",
  errorLanguageMissing: "Sprachdatei nicht gefunden.",
  errorAudioMissing: "Audiopaket nicht gefunden.",
  errorAudioUnsupported: "Audio-Upload wird nur auf dem ESP32-S3 AMOLED-Rosenkranz unterstützt.",
  errorFirmwareFailed: "Firmware-Update fehlgeschlagen.",
  errorIntentionsFailed: "Intentionen-Update fehlgeschlagen.",
  errorLanguageFailed: "Sprach-Update fehlgeschlagen.",
  errorAudioFailed: "Audio-Upload per Bluetooth fehlgeschlagen.",
  progressFirmware: "Firmware-Update",
  progressIntentions: "Intentionen-Update",
  progressLanguage: "Sprach-Update",
  progressAudio: "Audio-Upload per Bluetooth",
  completeFirmware: "Firmware-Update abgeschlossen.",
  completeIntentions: "Intentionen-Update abgeschlossen.",
  completeLanguage: "Sprach-Update abgeschlossen.",
  completeAudio: "Audio-Upload per Bluetooth abgeschlossen.",
  uiLangAria: "Interface-Sprache",
  usbFooter: "USB-Installer bereitgestellt von",
  footerAria: "Fußzeile",
  deployLabel: "Letzte Bereitstellung",
  changelogTitle: "Firmware-Änderungen",
  changelogVersionLabel: "Version:",
  changelogCommitLabel: "Release-Commit",
  changelogEmpty: "Keine Änderungsnotizen gefunden.",
  themeLightLabel: "Hell",
  themeDarkLabel: "Dunkel",
  themeLightTitle: "Auf helles Design umschalten",
  themeDarkTitle: "Auf dunkles Design umschalten",
  unsupportedMsg: "Ihr Browser unterstützt die Installation auf ESP-Geräten nicht. Bitte Google Chrome oder Microsoft Edge verwenden."
});
